...
if (!class_exists('ALOptimizer')) :
	...
	class ALOptimizer {
		...
		public function extractAndMatchStyles($buffer, $css_config) {
		...
		public function optimizeBuffer($buffer) {
			...
			if (is_array($template_config) && !empty($template_config)) {
				...
				if (array_key_exists('used_css', $template_config) && ($url_matched || $apply_used_css_on_similar_posts)) {
					$remaining_styles = $this->extractAndMatchStyles($buffer, $template_config['used_css_replace_rules']);
					if (!empty($remaining_styles)) {
						$page_audits['non_matching_stylesheets'] = $remaining_styles;
					}
		...
		public function doSearchAndReplace($buffer, $search_and_replace_info) {
			foreach ($search_and_replace_info as $config) {
				...
				if (array_key_exists("search_for_regex", $config) && !empty($config["search_for_regex"]) && array_key_exists("replace_with", $config)) {
					$buffer = ALHelper::safePregReplace($config["search_for_regex"], $config["replace_with"], $buffer);
				}
			}
			return $buffer;
		}
